		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				case IDC_ABOUT_WEB:
				{
					...
					const int Err = reinterpret_cast<int>( ShellExecute(hWnd, NULL, "http://www.footools.com/aboutplugin/uvwtorgb.html", NULL, NULL, NULL) );
					if (Err<32)
#else
	#if MAX_VERSION_MAJOR < 15	//Max 2013
					if ( ShellExecute(hWnd, NULL, "http://www.footools.com/aboutplugin/uvwtorgb.html", NULL, NULL, NULL) < (HINSTANCE) 32 )
	#else
					if ( ShellExecute(hWnd, NULL, _T("http://www.footools.com/aboutplugin/uvwtorgb.html"), NULL, NULL, NULL) < (HINSTANCE) 32 )
	#endif
#endif
					{
